...
describe('capture-debt route', () => {
  ...
  test('POST /capture-debt with no scheme selected returns "The scheme must be one of the following: SFI22, SFI Pilot, Lump Sums, Vet Visits, CS, BPS, FDMR, SFI23, Delinked, Expanded SFI Offer, Combined Offer Higher Tier Revenue, Combined Offer Higher Tier Capital." error message', async () => {
    const options = {
      method: 'POST',
      url: '/capture-debt',
      payload: { ...VALID_PAYLOAD, scheme: '' },
    ...
    const result = await server.inject(options)
    expect(result.request.response.source.context.model.errorSummary[0].text).toEqual('The scheme must be one of the following: SFI22, SFI Pilot, Lump Sums, Vet Visits, CS, BPS, FDMR, SFI23, Delinked, Expanded SFI Offer, Combined Offer Higher Tier Revenue, Combined Offer Higher Tier Capital.')
  })
  ...
  test('POST /capture-debt with invalid scheme returns "The scheme must be one of the following: SFI22, SFI Pilot, Lump Sums, Vet Visits, CS, BPS, FDMR, SFI23, Delinked, Expanded SFI Offer, Combined Offer Higher Tier Revenue, Combined Offer Higher Tier Capital." error message', async () => {
    const options = {
      method: 'POST',
      url: '/capture-debt',
      payload: { ...VALID_PAYLOAD, scheme: 'not-a-scheme' },
  ...
  test('POST /capture-debt returns 400 when the date payload is after the date that the debt was discovered', async () => {
    ...
    const result = await server.inject(options)
    expect(result.statusCode).toBe(400)
    expect(result.request.response.source.context.model.errorSummary[0].text).toEqual('Date cannot be in the future.')
  })
