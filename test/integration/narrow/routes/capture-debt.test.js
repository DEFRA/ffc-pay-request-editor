describe('capture-debt route', () => {
  ...
  beforeEach(async () => {
    ...
    getSchemeId.mockResolvedValue(SCHEME_NAME_SFI)
  ...
  test('POST /capture-debt with valid payload is saved to debtData table', async () => {
    ...
    // const { SCHEMES, SCHEME_ID_SFI } = require('../../../data/scheme-id')

    // await db.sequelize.truncate({ cascade: true })

    // await db.scheme.create(SCHEMES[0])
    // const sfi = db.scheme.build({ schemeId: 1, name: 'SFI' })
    // await sfi.save().catch(e => {
    //   console.log(e)
    // })
    // await db.scheme.create({ schemeId: 1, name: 'SFI' })

    await db.paymentRequest.create({ schemeId: 1, frn: VALID_PAYLOAD.frn }).catch(e => { console.log(e) })
