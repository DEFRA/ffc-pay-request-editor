...
describe('capture-debt route', () => {
  ...
  test('POST /capture-debt with no scheme selected returns "The scheme must be one of the following: SFI, SFI Pilot, CS, BPS, FDMR, Vet Visits, Lump Sums." error message', async () => {
    const options = {
      method: 'POST',
      url: '/capture-debt',
      payload: { ...VALID_PAYLOAD, scheme: '' },
    ...
    const result = await server.inject(options)
    expect(result.request.response.source.context.model.errorSummary[0].text).toEqual('The scheme must be one of the following: SFI, SFI Pilot, CS, BPS, FDMR, Vet Visits, Lump Sums.')
  })
  ...
  test('POST /capture-debt with invalid scheme returns "The scheme must be one of the following: SFI, SFI Pilot, CS, BPS, FDMR, Vet Visits, Lump Sums." error message', async () => {
    const options = {
      method: 'POST',
      url: '/capture-debt',
      payload: { ...VALID_PAYLOAD, scheme: 'not-a-scheme' },
