...
describe('Get payment request test', () => {
  ...
  test('should return zero payment requests with categoryId 2', async () => {
    ...
    expect(paymentRequests.rows).toHaveLength(0)
  })
  ...
  test('should create a receivedFormatted virtual type when paymentRequest has a received value', async () => {
    ...
    expect(typeof paymentRequests.rows[0].receivedFormatted).toBe('string')
    expect(paymentRequests.rows[0].receivedFormatted).not.toBe(null)
    expect(paymentRequests.rows[0].receivedFormatted).toBe(
      paymentRequests.rows[0].received.toLocaleDateString('en-GB', {
        year: 'numeric',
        month: '2-digit',
        day: '2-digit'
      })
  ...
  test('daysWaiting virtual type should return the number of days since the "received" date', async () => {
    ...
    expect(paymentRequests.rows[0].daysWaiting).toBe(daysWaiting)
    expect(paymentRequests.rows[0].daysWaiting).not.toBe(null)
    expect(typeof paymentRequests.rows[0].daysWaiting).toBe('number')
  })
  ...
  test('daysWaiting virtual type should return "" when paymentRequest has no "received" value', async () => {
    ...
    expect(paymentRequests.rows[0].daysWaiting).toBe('')
  })
  ...
  test('records should be returned ordered by "received" in ascending order', async () => {
    ...
    expect(paymentRequestRows.rows[0].received)
      .toStrictEqual(new Date('2022-01-01T00:00:00.000Z'))

    expect(paymentRequestRows.rows[1].received)
      .toStrictEqual(new Date('2022-04-01T00:00:00.000Z'))
  })
  ...
  test('get payment request for enrichment should match by agreement number if not CS', async () => {
    const matchedPaymentRequest = await getPaymentRequestAwaitingEnrichment(
      ...
      paymentRequest.categoryId
    )

    expect(matchedPaymentRequest.agreementNumber)
      .toBe(paymentRequest.agreementNumber)
  })
  ...
  test('get payment request for enrichment should match by contract number if CS', async () => {
    ...
    expect(matchedPaymentRequest.contractNumber)
      .toBe(paymentRequest.contractNumber)
  })
